...
=======
  html_document:
    ...
    toc_float:
      collapsed: false
>>>>>>> 09e6e28219c5773ed94e23251d27e9ee46cbdf93
---
...
hist(morn_visits$sumvisits) ## left-biased distribution, aka Poisson
mean(morn_visits$sumvisits)

hist(morn_visits$nightlows) ## fairly Gaussian, but not perfect, may need to rescale if model fit is off
mean(morn_visits$nightlows)

hist(morn_visits$Date, breaks = "day") ## data distribution over the study is fairly even, except for a gap when there were technological issues with the RFID feeders
```
...
temp$Predict <- predict(glm.1, temp, type = "response") # takes parameters from model and returns values for the response variable
...
bootstrap <- function(data, samp_size, calc_statistic, b_iter) {
  boot_statistic <- function() {
    boot_sample = sample(x = data, size = samp_size, replace = T) ## randomly sample rows with replacement; enter sample values as samp_size, up to the original sample size
    calc_statistic(boot_sample) ## apply the statistic of interest
  }
  
  replicate(n = b_iter, expr = boot_statistic())  ## perform the bootstrap multiple times by changing b_iter
}
set.seed(457)
...
```{r}
library(tidyverse)
glmdf = data.frame(Estimate = numeric()) ##initialize dataframe for our output parameters
for(i in 1:1000){
  sampdata = sample_n(morn_visits, size = nrow(morn_visits), replace = TRUE) ## randomly sample rows with replacement up to the original sample size
  glm_temp = glm(formula = sumvisits ~ nightlows, data = sampdata, family = "poisson") ## model the effects of interest
  glmdf = rbind(glmdf, data.frame(Estimate = summary(glm_temp)$coefficients[, 1][2])) ## save the output to our dataframe
}

quantile(glmdf$Estimate,probs = c(0.025,0.975)) ## get confidence intervals; cool, they do not overlap with zero! That would mean we cannot reject the null hypothesis--i.e., that there is no relationship between the response and predictor variables

hist(glmdf$Estimate) ## look at the distribution of the parameter
